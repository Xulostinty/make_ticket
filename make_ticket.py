...
# 黑字 = 纯黑(用户要求)。原来是从参考扫描件上量的暗紫灰 (78,71,87) —— 扫描把油墨的
# 边缘糊开、纸基的蓝也混进去, 量出来偏灰紫。想要回扫描件的观感就把这行改回
...
LAYOUT = {
    ...
    "gate_code":  (86.51, 0.9755, 1787.68, 96.31, "Camb", "18B"),
    "from_name":  (128.19, 1.0524, 221.18, 252.60, "HeiR", "北京南"),
    "from_zhan":  (69.63, 1.1590, 642.89, 233.50, "ZS", "站"),
    "from_py":    (80.51, 0.9442, 310.71, 333.95, "Times", "Beijingnan"),
    "train_no":   (124.19, 1.0162, 934.59, 253.13, "Times", "G99"),
    "to_name":    (125.94, 1.0907, 1277.56, 249.72, "HeiR", "南京南"),
    "to_zhan":    (68.55, 1.1467, 1701.95, 230.62, "ZS", "站"),
    "to_py":      (81.69, 0.9430, 1448.38, 332.77, "Times", "Nanjingnan"),
    "date_y":     (104.98, 1.0000, 156.45, 451.00, "Digits", "2026"),
    "date_yn":    (56.08, 0.9611, 355.93, 429.99, "ZS", "年"),
    "date_m":     (103.63, 1.0000, 438.52, 450.00, "Digits", "01"),
    "date_mn":    (62.22, 0.8869, 545.21, 431.82, "ZS", "月"),
    "date_d":     (104.98, 1.0000, 628.79, 450.00, "Digits", "01"),
    "date_dn":    (58.89, 1.0018, 729.99, 430.55, "ZS", "日"),
    "date_hm":    (108.43, 1.0000, 818.38, 449.57, "Digits", "09:00"),
    "date_kai":   (55.31, 0.9936, 1062.85, 429.45, "ZS", "开"),
    "coach":      (104.98, 1.0000, 1323.89, 449.00, "Digits", "05"),
    "coach_che":  (58.02, 0.9336, 1414.22, 430.89, "ZS", "车"),
    "seat":       (104.98, 1.0000, 1488.89, 449.00, "Digits", "12F"),
    "seat_hao":   (57.60, 0.9546, 1627.89, 428.92, "ZS", "号"),
    "price_yen":  (104.52, 0.7306, 162.48, 554.41, "Sun", "￥"),
    "price_int":  (100.40, 1.0000, 246.40, 554.60, "Digits", "553."),
    ...
    **默认值是一张完全虚构的票**（北京南 → 上海虹桥 G1，2026-01-01 09:00，05车12A号，
    ...
    def from_invoice(cls, path, use_qr_image=True):
        ...
        return cls(
            ...
            gate=None,
            from_station=(d.get("from_station") or "北京南").rstrip("站"),
            from_pinyin=d.get("from_pinyin") or "",
            to_station=(d.get("to_station") or "上海虹桥").rstrip("站"),
            to_pinyin=d.get("to_pinyin") or "",
            train_no=d.get("train") or "G1",
            depart=_dt.datetime(y, m, dd, hh, mi),
            coach=d.get("coach") or "05",
            seat=d.get("seat") or "",
